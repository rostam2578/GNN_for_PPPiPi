...
for BatchSize, LrVal, weight_decay_val in hpmesh[startmesh:endmesh]:
  ...
  batcharray = np.arange(1 * TraEvN, 2 * TraEvN, BatchSize)
  evalbatchnum = int((TraEvN // BatchSize) * 0.06) + 1
  evalbatcharray = np.random.choice(batcharray, evalbatchnum)
  purity = np.zeros(shape = (2 * TraEvN, pointnum))
  efficiency = np.zeros(shape = (2 * TraEvN, pointnum))
  eval_purity = np.zeros(shape = (2 * TraEvN, pointnum))
  eval_efficiency = np.zeros(shape = (2 * TraEvN, pointnum))
...
fig.colorbar(ax3.matshow(sitonsquare(result1.reshape(6796) > -0.3), aspect=2, extent=[0, 288, 0, 43], origin='lower')\
                ,fraction=0.014)
plt.xlabel('cell')
plt.ylabel('layer')
plt.xlim(0, 288)
...
fig.colorbar(ax4.matshow(sitonsquare(traingnnpppipi[EvBTr + 10]), aspect=2, vmin=0, vmax=1, extent=[0, 288, 0, 43], origin='lower')\
                ,fraction=0.014)
plt.xlabel('cell')
plt.ylabel('layer')
plt.xlim(0, 288)
...
fig.colorbar(ax5.matshow(sitonsquare(trvalgnnpppipi[EvBTr + 10]), aspect=2, vmin=0, vmax=1, extent=[0, 288, 0, 43], origin='lower')\
                ,fraction=0.014)
plt.xlabel('cell')
plt.ylabel('layer')
plt.xlim(0, 288)
plt.ylim(0, 43)
plt.title(f'event number {EvBTr + 10} without noise \n color indicates time')

# pass from the net
ax6 = fig.add_subplot(336)
fig.colorbar(ax6.matshow(sitonsquare(result2.reshape(2, 6796)[0] > 0.7), aspect=2, extent=[0, 288, 0, 43], origin='lower')\
                ,fraction=0.014)
plt.xlabel('cell')
plt.ylabel('layer')
plt.xlim(0, 288)
plt.ylim(0, 43)
plt.title(f'event {EvBTr + 10} passed through network in a batch after training \n color indicates time')

# the second event
ax7 = fig.add_subplot(337)
fig.colorbar(ax7.matshow(sitonsquare(traingnnpppipi[EvBTr + 11]), aspect=2, vmin=0, vmax=1, extent=[0, 288, 0, 43], origin='lower')\
                ,fraction=0.014)
plt.xlabel('cell')
plt.ylabel('layer')
plt.xlim(0, 288)
...
fig.colorbar(ax8.matshow(sitonsquare(trvalgnnpppipi[EvBTr + 11]), aspect=2, vmin=0, vmax=1, extent=[0, 288, 0, 43], origin='lower')\
                ,fraction=0.014)
plt.xlabel('cell')
plt.ylabel('layer')
plt.xlim(0, 288)
plt.ylim(0, 43)
plt.title(f'event number {EvBTr + 11} without noise \n color indicates time')

# pass from the net
ax9 = fig.add_subplot(339)
fig.colorbar(ax9.matshow(sitonsquare(result2.reshape(2, 6796)[1] > 0.5), aspect=2, extent=[0, 288, 0, 43], origin='lower')\
                ,fraction=0.014)
plt.xlabel('cell')
plt.ylabel('layer')
plt.xlim(0, 288)
plt.ylim(0, 43)
plt.title(f'event {EvBTr + 11} passed through network in a batch after training \n color indicates time')

#########################################################
plt.savefig(f'/hpcfs/bes/mlgpu/hoseinkk/MLTracking/otherparticles/pppipi/{modelname}/results/{t}\
    passing three random events {EvBTr, EvBTr + 10, EvBTr + 11} from network after training.png', bbox_inches='tight')
