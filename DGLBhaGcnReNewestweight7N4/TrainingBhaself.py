...
for BatchSize, LrVal, weight_decay_val in hpmesh[startmesh:endmesh]:
  ...
  batcharray = np.arange(3 * TraEvN, 4 * TraEvN, BatchSize)
  evalbatchnum = int((TraEvN // BatchSize) * 0.06) + 1
  evalbatcharray = np.random.choice(batcharray, evalbatchnum)
  purity = np.zeros(shape = (4 * TraEvN, pointnum))
  efficiency = np.zeros(shape = (4 * TraEvN, pointnum))
  eval_purity = np.zeros(shape = (4 * TraEvN, pointnum))
  eval_efficiency = np.zeros(shape = (4 * TraEvN, pointnum))
